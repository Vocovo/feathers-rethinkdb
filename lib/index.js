...
const profile = (logger, start, query) => {
  try{
    logger.debug(query.toString(), `${(new Date()).getTime()-start}ms`)
  } catch(e) {
    logger.debug('Query could not be output due to error')
  }
}
...
class Service {
  ...
  createQuery (originalQuery) {
    ...
    // Handle table secondary indexes
    if (this.options.tableIndex) {
      _.each(this.options.tableIndex, (order, fieldName) => {
        if (parseInt(order) === 1) {
    ...
    // Handle $sort
    if (!this.options.tableIndex && filters.$sort) {
      _.each(filters.$sort, (order, fieldName) => {
        let ra
        
        const fields = fieldName.split('.')
        if (fields.length > 1) ra = r.row

        fields.forEach(field => {
          // Use the ReThink r.row(field)(nestedfield)(...) notation
          // for nested fields.
          if (ra) ra = ra(field)
          else ra = field
        })
  ...
  _find (params = {}) {
    ...
    // Counting large filter()s is dog-slow though, so in cases where a total record count 
    // is non-essential, but paging the results must still occur, we can turn this off
    //
    // `paginate.noCount` is a way to explictly request no counting per query.
    if (paginate.default && !paginate.noCount) {
    ...
    // Execute the query
    return Promise.all([q, countQuery])
      .then(res => {
        // profile query performance if in debug mode
        this.debug && profile(this.logger, start, q)
        return res
      })
      .then(([data, total]) => {
        if (paginate.default) {
